...
-- All tables are prefixed with 'language_app_' to avoid conflicts with other apps

-- Create categories table for better organization
CREATE TABLE language_app_categories (
...
CREATE TABLE language_app_difficulty_levels (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  name TEXT NOT NULL UNIQUE,
...
CREATE TABLE language_app_french_phrases (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  text TEXT NOT NULL,
  english_translation TEXT NOT NULL,
  difficulty_id UUID REFERENCES language_app_difficulty_levels(id) ON DELETE CASCADE,
  category_id UUID REFERENCES language_app_categories(id) ON DELETE CASCADE,
...
CREATE INDEX idx_language_app_french_phrases_difficulty ON language_app_french_phrases(difficulty_id);
CREATE INDEX idx_language_app_french_phrases_category ON language_app_french_phrases(category_id);
CREATE INDEX idx_language_app_french_phrases_active ON language_app_french_phrases(is_active);
CREATE INDEX idx_language_app_french_phrases_difficulty_score ON language_app_french_phrases(difficulty_score);
CREATE INDEX idx_language_app_french_phrases_frequency_score ON language_app_french_phrases(frequency_score);
...
CREATE TRIGGER update_language_app_categories_updated_at BEFORE UPDATE ON language_app_categories
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_language_app_difficulty_levels_updated_at BEFORE UPDATE ON language_app_difficulty_levels
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_language_app_french_phrases_updated_at BEFORE UPDATE ON language_app_french_phrases
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Insert initial data
INSERT INTO language_app_categories (name, description, color) VALUES
...
INSERT INTO language_app_difficulty_levels (name, level_order, description, color) VALUES
...
INSERT INTO language_app_french_phrases (text, english_translation, difficulty_id, category_id, difficulty_score, frequency_score) VALUES
  ...
  ('Je m''appelle Marie', 'My name is Marie',
   (SELECT id FROM language_app_difficulty_levels WHERE name = 'beginner'),
   (SELECT id FROM language_app_categories WHERE name = 'introductions'), 1, 8),
  ...
  ('Où est la gare ?', 'Where is the train station?',
   (SELECT id FROM language_app_difficulty_levels WHERE name = 'beginner'),
   (SELECT id FROM language_app_categories WHERE name = 'directions'), 3, 6),
  ...
  ('Je voudrais réserver une table pour deux personnes', 'I would like to reserve a table for two people',
   ...
   (SELECT id FROM language_app_categories WHERE name = 'restaurant'), 5, 4),
  
  ('Pouvez-vous m''aider, s''il vous plaît ?', 'Can you help me, please?',
   (SELECT id FROM language_app_difficulty_levels WHERE name = 'intermediate'),
   (SELECT id FROM language_app_categories WHERE name = 'requests'), 4, 7),
  ...
  ('Malgré les difficultés, nous avons réussi', 'Despite the difficulties, we succeeded',
   (SELECT id FROM language_app_difficulty_levels WHERE name = 'advanced'),
   (SELECT id FROM language_app_categories WHERE name = 'achievements'), 7, 4),
  
  ('Il est essentiel de bien comprendre cette situation', 'It is essential to understand this situation well',
   (SELECT id FROM language_app_difficulty_levels WHERE name = 'advanced'),
   (SELECT id FROM language_app_categories WHERE name = 'analysis'), 9, 2),
